...
def get_number_and_coordinates(data: list[list[str]]) -> Iterator[int]:
    number_digits = []
    coordinates = []
    for row_id, row in enumerate(data):
        for elem_id, elem in enumerate(row):
            if elem.isdigit():
                number_digits.append(elem)
                coordinates.append((row_id, elem_id))
                continue

            if number_digits:
                yield int(''.join(number_digits)), coordinates
                number_digits = []
                coordinates = []


def adjacent_to_symbol(data, x_coord, y_coord) -> bool:
    length = len(data)
    width = len(data[0])
    for x in (x_coord - 1, x_coord, x_coord + 1):
        for y in (y_coord - 1, y_coord, y_coord + 1):
            ...
            if x not in range(width) or y not in range(length):
                continue
...
def convert_input_to_2d_list(data: str) -> list[list[str]]:
    _, *lines = data.splitlines()
    list_2d = []
    for line in lines:
        list_2d.append(list(line))
    return list_2d
...
if __name__ == '__main__':
    ...
    engine_sum = 0
    for value, number_coordinates in get_number_and_coordinates(data_in_2d_list):
        if any([adjacent_to_symbol(data_in_2d_list, x, y) for x, y in number_coordinates]):
            engine_sum += value
    print(engine_sum)
